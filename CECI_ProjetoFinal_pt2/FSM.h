int Adr,Instr,SrcA,SrcB,aluSrcB[2],AlUOut,r=0,PCEn;
...
void setRegisters(int* reg_s, int* reg_t, int* reg_d)
{
	int i;
	for(i = 0; i < 5; ++i){
		rs[i] = reg_s[i];
		rt[i] = reg_t[i];
		rd[i] = reg_d[i];
		printf("rs[%i]: %i\t", i, rs[i]);
		printf("rt[%i]: %i\t", i, rt[i]);
		printf("rd[%i]: %i\n", i, rd[i]);
	}
}
		...
		if (rst){ 
			...
			switch(estado){
				...
				case S6: 
					...
					proxEstado = S7;
					Execute(clk, clka, FSMMemToReg, funct, AluControl, FSMAluSrcA, FSMAluSrcB, rs, rt, rd);
					break;
				case S6_0: 
				case S6_1: 
				case S6_2: 
...
void AfctFSM(int reset, int opcode, int Funct){
	...
	if (FSMPCWrite == 1 || (FSMBranch && FSMALUZero)) 
		PCEn = 1;
	else 
		PCEn = 0;
